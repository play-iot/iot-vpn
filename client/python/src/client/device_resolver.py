...
class DNSResolver:
    ...
    def __init__(self, unix_service: UnixService):
...
class SystemdService(UnixService):
    ...
    def create(self, opts: UnixServiceOpts, service_tmpl_path: str, replacements: dict):
        ...
        FileHelper.copy(service_tmpl_path, service_fqn)
        FileHelper.replace_in_file(service_fqn, replacements, backup='')
        FileHelper.chmod(service_fqn, mode=0o644)
        logger.info(f"Enable System service '{opts.service_name}'...", )
...
class DeviceResolver:
    ...
    def probe(self) -> 'DeviceResolver':
        ...
        self._dns_resolver(DNSResolver(self.unix_service).probe())
        return self
