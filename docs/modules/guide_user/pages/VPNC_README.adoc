...
* After connect VPN connection (See <<_vpn_workflow,VPN workflow>>), VPN client will be registered as linux service, then you can manage it as normal linux service with some basic linux commands
+
[,bash]
----
...
. For upgrade to new version, download the latest version as <<_download_and_setup_vpn_client,instruction>>, then use:
+
[,bash]
----
  $ playio-vpnc upgrade
...
* Must use `xref:cmd.adoc[Client Certificate Authentication]`
